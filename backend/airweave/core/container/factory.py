...
from airweave.domains.embedders.sparse.null import NullSparseEmbedder
...
def _create_sparse_embedder(registry: SparseEmbedderRegistry) -> SparseEmbedderProtocol:
    ...
    # Use the embedder class from the spec (supports null/other embedders)
    if spec.embedder_class_ref == NullSparseEmbedder:
...
def _build_llm_chain(
    ...
    for provider, model in llm_fallback_chain:
        ...
        # For OPENAI provider, skip API key check if OPENAI_BASE_URL is configured (local LLM)
        if provider == LLMProvider.OPENAI and settings.OPENAI_BASE_URL:
            pass  # Allow local LLM without API key
        elif api_key_attr and not getattr(settings, api_key_attr, None):
