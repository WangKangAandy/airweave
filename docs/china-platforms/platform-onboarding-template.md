...
- 图标文件必须来自可追溯网络来源（官方品牌页或可信 logo 站点），禁止手绘、截图裁切、临时拼接后直接入库。
- 图标文件固定路径：`frontend/src/components/icons/apps/<platform>.svg`，优先使用 SVG，不使用位图嵌入替代 SVG 资产。
- 提交前必须在接入文档记录图标来源 URL（可复查），避免后续无法确认版权与真伪。
- 如果首选来源下载失败（404/429/超时）：
  1) 切换到第二可信来源继续拉取；
  2) 明确记录失败原因与尝试过的 URL；
  3) 不得用“用户随手截图”或“人工临摹”作为最终版本。
- 验证要求：本地刷新后图标视觉与官方一致，且文件内容为纯 SVG 路径/矢量元素（非 base64 截图嵌入）。
